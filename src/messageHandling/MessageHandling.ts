...
import {handleYouTubeShorts} from "./handlers/YouTube_Shorts";

export type MessageHandler = (client: Client, message: Message) => boolean;

const messageHandlers: MessageHandler[] = [handleYouTubeShorts];
...
export const applyMessageHandlers = (client: Client, message: Message) => {
    for (const handler of messageHandlers) {
        ...
        if (handler(client, message)) {
            console.log(`Replied to message ID: ${message.id}`);
            break;
        }
    }
};
